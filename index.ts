...
const QUOTED_STRING_TEST = (remainingString: string) =>
  remainingString.charAt(0) === "{";
const QUOTED_STRING_GET_AND_ADVANCE = (
...
const I18N_PIPES = {
  plural: (value: any, options: string[], values: string[]) => {
    ...
    const mapper: { [option: string]: string } = options.reduce(
      (map, option, index) => ({
        ...map,
        [option.charAt(0) === "="
          ? option.slice(1).trim()
          : option.trim()]: values[index],
      }),
      {}
    );
    const numValue = +value;
    ...
    if (numValue === numValue) {
      ...
      return mapper.hasOwnProperty(value)
        ? mapper[value]
        : [
            ...(numValue >= 10 && numValue < 20
              ? ["many"]
              : lastDigit[numValue % 10] || ["many"]),
            ...(numValue >= 20 ? ["many"] : []),
            "other",
          ]
            .filter((key) => mapper.hasOwnProperty(key))
            .map((key) => mapper[key])[0] || defValue;
    }
    return mapper["other"] == null ? defValue : mapper["other"];
  },
  select: (value: any, options: string[], values: string[]) => {
    ...
    options = options
      ...
      .slice(0, options.length - 1);
    const mapper: { [option: string]: string } = options.reduce(
      (map, option, index) => ({ ...map, [option.trim()]: values[index] }),
      {}
    );
    return mapper[value] == null ? defValue : mapper[value];
  },
};
...
export class TranslateICUCustomParser {
  ...
  constructor(
    private additionalPipes?: { [pipeName: string]: ICUPipeFunction }
  ) {
    this.pipeEntries = Object.entries({
      ...(additionalPipes || {}),
      ...I18N_PIPES,
    });
  }

  private getPipes(variables: {
    [varName: string]: string;
  }): { [pipeName: string]: PipeFunction } {
    return this.pipeEntries
      .map(([pipeName, pipe]) => ({
        [pipeName]: (value: any, [parameter]: string[]) => {
          const parsedParameter = parseI18NParameter(parameter);
          const options = parsedParameter.literals;
          const values = parsedParameter.variables.map((variable) =>
            this.interpolate(variable.name, variables)
          );
